...
int serial_open_advanced(serial_t *serial, const char *path, uint32_t baudrate, unsigned int databits, serial_parity_t parity, unsigned int stopbits, bool xonxoff, bool rtscts) {
    ...
    if (parity != PARITY_NONE && parity != PARITY_ODD && parity != PARITY_EVEN && parity != PARITY_SPACE && parity != PARITY_MARK)
        return _serial_error(serial, SERIAL_ERROR_ARG, 0, "Invalid parity (can be PARITY_NONE,PARITY_ODD,PARITY_EVEN,PARITY_MARK,PARITY_SPACE)");
...
//    ISTRIP caused some errors
    ...
    if (parity == PARITY_EVEN)
        ...
        termios_settings.c_cflag |= (PARENB | CMSPAR);
        termios_settings.c_cflag &= ~PARODD;
    }
    else if (parity == PARITY_MARK)
        termios_settings.c_cflag |= (PARENB | CMSPAR | PARODD);
...
// PARENB is 1
    if ((termios_settings.c_cflag & PARENB) == 0)
        *parity = PARITY_NONE;
// PARENB is 1 and
    else if ((termios_settings.c_cflag & CMSPAR) == 0){
        if ((termios_settings.c_cflag & PARODD) == 0)
            *parity = PARITY_EVEN;
        else
            *parity = PARITY_ODD;
    } else {
...
int serial_set_parity(serial_t *serial, enum serial_parity parity) {
    ...
    if (parity != PARITY_NONE && parity != PARITY_ODD && parity != PARITY_EVEN && parity != PARITY_MARK && parity != PARITY_SPACE)
...
//        termios_settings.c_iflag |= (INPCK | ISTRIP);
    ...
    termios_settings.c_cflag &= ~(PARENB | PARODD | CMSPAR);
    if (parity == PARITY_EVEN)
        termios_settings.c_cflag |= PARENB;
    else if (parity == PARITY_ODD)
        termios_settings.c_cflag |= (PARENB | PARODD);
